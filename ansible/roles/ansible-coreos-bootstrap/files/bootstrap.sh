...
$SUDO "$PYPY_INSTALL/bin/pypy" -m ensurepip
$SUDO "$PYPY_INSTALL/bin/pypy" -mpip install -U pip wheel

$SUDO ln -sf "$PYPY_INSTALL/bin/python" "$PKG_HOME/bin/python"
$SUDO ln -sf "$PYPY_INSTALL/bin/pip" "$PKG_HOME/bin/pip"
$SUDO ln -sf "$PYPY_INSTALL/bin/wheel" "$PKG_HOME/bin/wheel"
...
PIP_VERSION=`$PYPY_INSTALL/bin/pip --version | awk '{ print $2 }'`
WHEEL_VERSION=`$PYPY_INSTALL/bin/wheel version | awk '{ print $2 }'`

# install needed modules
$PYPY_INSTALL/bin/pip install docker-py
